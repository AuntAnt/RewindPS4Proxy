...
func InitProxy() *goproxy.ProxyHttpServer {
	...
	proxy.Verbose = false
	proxy.OnRequest().DoFunc(ClientRequest) //mapping url
	proxy.OnRequest().HandleConnectFunc(func(host string, ctx *goproxy.ProxyCtx) (*goproxy.ConnectAction, string) {
		...
		latestClientUserAgent = userAgent
		clientActive = true
...
func CheckClientActivity() {
	go func() {
		for {
			...
			if clientActive {
				clientActive = false // reset the client activity status
			} else {
...
func SetMode(mode int, jsonLink string) (string, error) {
	if mode == 1 {
		gameDetails, err := Details(jsonLink)
		if err != nil {
		...
		Logger("INFO", fmt.Sprintf("URL mapping:\n[%s]%s\nwill be mapped to\n[%s]%s", ExtractVersion(lastJSONURL), lastJSONURL, ExtractVersion(jsonLink), jsonLink))
		Logger("INFO", fmt.Sprintf("MODE1 (JSON mapping) enabled.\nGame: %s\nDowngrade version: %s", gameName, ExtractVersion(jsonLink)))
...
func GetLastServerError() string {
	if lastError != nil {
...
func ClientRequest(req *http.Request, ctx *goproxy.ProxyCtx) (*http.Request, *http.Response) {
	...
	if latestClientIP != clientIP || latestClientUserAgent != userAgent {
		latestClientIP = clientIP
		latestClientUserAgent = userAgent
		clientActive = true
		deviceType := Detector(userAgent)
	...
	if newURL, ok := urlMap[baseURL]; ok {
		...
		Logger("INFO", fmt.Sprintf(`URL mapping successful!%s --> %s`, ExtractVersion(baseURL), ExtractVersion(newURL)))
		return req, nil
	}
...
func ClientStatus(w http.ResponseWriter, r *http.Request) {
	...
	response := map[string]interface{}{
		...
		"latestClientIP":        latestClientIP,
		"latestClientUserAgent": latestClientUserAgent,
	}
	jsonResponse, err := json.Marshal(response)
	if err != nil {
